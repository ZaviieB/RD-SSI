...
for i = 1 
    ...
    title('Fourier amplitude specture');
...
switch SeparateType
    case 1  % 指定範圍
        CutFrequency_min =   0.74   ; %不切割min  
        CutFrequency_max =    1.117  ; %不切割max
        
    case 2  % 指定間距  %CutFrequency = [ ] 填頻率 %CutFrequency_range = [ ] 填與左間距，往右切割對稱

	CutFrequency=	1.85335	;	%振態頻率
CutFrequency_range = 0.077 ;    
...
%%    
    axis([0 600 -inf inf]); %TimeHistory %RD前歷時圖
...
%%
mode_1 = time_history(0/dt+1:599.9/dt,1); %%% 改自由震盪的範圍 %取0-600秒-400
%Td = (t(599.9/dt)-t(0/dt+1))/12; 
Td = 50;
...
yTimeHistory=round(max(time_history),2);
yRDSTD=round(std(N1),2);
ymaxValue = round(max(N1),2);
%%
